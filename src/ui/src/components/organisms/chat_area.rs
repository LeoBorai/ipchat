...
pub fn ChatArea(#[prop(into)] username: Signal<String>) -> impl IntoView {
    ...
    view! {
        <div class="flex-1 flex flex-col">
            <Show when=move || active_room.get().is_some() fallback=move || {
                view! {
                ...
                <div class="bg-white border-b border-gray-200 p-4">
                    <h2 class="font-bold text-lg text-gray-800">{use_active_room().get().unwrap().name.clone()}</h2>
                ...
                <div class="bg-white border-t border-gray-200 p-4">
                    <div class="flex gap-2">
                        <input
                            ...
                            on:keypress={
                                let handle_send_message = handle_send_message;
                                move |ev| {
                                    if ev.key() == "Enter" {
                                        handle_send_message();
                                    }
