...
pub fn ChatArea(
    ...
    messages: Signal<HashMap<String, Vec<Message>>>,
    send_message: impl Fn(String, String, String) + 'static + Copy,
) -> impl IntoView {
    ...
    let handle_send_message = move || {
        ...
        if !message_text.trim().is_empty() {
            if let Some(room) = active_room.get() {
                send_message(room.id, message_text, username.clone());
                                ...
                                <div class="flex-1 overflow-y-auto p-4 space-y-3">
                                    {move || {
                                        get_room_messages()
                                            ...
                                            .enumerate()
                                            .map(|(idx, msg)| {
                                                let is_own = msg.sender == username;
